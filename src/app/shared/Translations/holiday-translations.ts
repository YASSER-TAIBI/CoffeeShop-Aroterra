export const HolidayTranslations: { [key: string]: string } = {
  //Maroc
  "Eid al-Fitr Holiday": "Eid al-Fitr",
  "Eid al-Fitr": "Eid al-Fitr",
  "Eid al-Fitr (tentative)": "Eid al-Fitr",
  "Eid al-Fitr Holiday (tentative)": "Eid al-Fitr",
  "Eid al-Adha Holiday": "Eid al-Adha",
  "Eid al-Adha": "Eid al-Adha",
  "Eid al-Adha (tentative)": "Eid al-Adha",
  "Eid al-Adha Holiday (tentative)": "Eid al-Adha",
  "Amazigh New Year": "Nouvel An amazigh",
  "Labour Day/May Day": "Fête du Travail",
  "Youth Day": "Journée de la jeunesse",
  "Anniversary of the Green March": "Anniversaire de la Marche Verte",
  "New Year's Day": "Jour de l'An",
  "Anniversary of the Independence Manifesto": "Anniversaire du Manifeste de l'Indépendance",
  "Feast of the Throne": "Fête du Trône",
  "Anniversary of the Recovery Oued Ed-Dahab": "Anniversaire de la Reconstruction d'Oued Ed-Dahab",
  "Anniversary of the Revolution of the King and the People": "Anniversaire de la Révolution du Roi et du Peuple",
  "Independence Day": "Jour de l'indépendance",
  "Hijra New Year": "Nouvel An Hijra",
  "Hijra New Year (tentative)": "Nouvel An Hijra",
  "The Prophet Muhammad's Birthday": "Anniversaire du prophète mohamed",
  "The Prophet Muhammad's Birthday (tentative)": "Anniversaire du prophète mohamed",
  "The Prophet Muhammad's Birthday Holiday": "Anniversaire du prophète mohamed",
  "The Prophet Muhammad's Birthday Holiday (tentative)": "Anniversaire du prophète mohamed",

  //France
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween",
  "halloween": "Halloween"

  // Ajoutez toutes les autres traductions ici
};
